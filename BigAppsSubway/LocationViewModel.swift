...
extension LocationViewModel: CLLocationManagerDelegate {
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        rider.coordinate = locations.last
        
        guard let napperLocation = rider.coordinate else { print("didUpdateLocations - error getting napper coordinate"); return }
        print("didUpdateLocations - napper coordinate = \(napperLocation)")
        
        
        if rider.destination.count > 0 {
            
            let nextDestination = rider.destination[0]
